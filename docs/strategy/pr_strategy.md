...
- 병합은 관리자가 진행하며, 승인된 PR만 `develop`으로 병합합니다.
...
### 3. 리뷰 요청
리뷰어를 지정하고 GitHub Copilot에도 리뷰를 요청합니다. <br>수정 요청이 있으면 작업 브랜치에 추가 커밋을 푸시합니다.

### 4. 병합
리뷰 승인 후 PR을 `develop`으로 병합하고, 병합된 작업 브랜치는 삭제합니다.
