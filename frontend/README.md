...
| 공간 진단 | 개선 방안 3개, 7일 일정 4개는 화면 내부 고정 문구. 선택 작물명은 정적 `crops` 코드표로 변환 | `app/src/screens/analysis/AnalysisScreen.tsx`, `app/src/data.ts` | 보고서 공간명은 기기 API의 연결 공간 응답, 환경 점수 API factor(온도·습도·조도·토양 수분·토양 온도)와 최신 측정값·토양 추천·대체 작물 추천은 API. 토양 진단 행과 상태는 score API factor를 한 번만 사용하며, 요인별 권장 조치 문구는 `shared/factorPresentation.ts`의 정적 매핑 |
| 실시간 모니터링 | 지표명·단위·색상·표시 구간(`liveMetricDefinitions`) | `app/src/data.ts` | 현재값, 최저·최고값, 5개 시계열은 API. 정적 값 데이터는 없음 |
| 관리 가이드 | 오늘 작업 2개(`managementTasks`), 재배 단계 기준 3개(`cultivationCriteria`), 추천 상품의 이름·가격·설명(`shopProducts`), 환경 요인→상품 ID 매핑(`factorProductMap`) | `app/src/data.ts`, `app/src/shared/factorPresentation.ts` | 토양 배합 추천과 환경 점수는 API. 작업 완료 체크는 화면 메모리 상태이며 저장되지 않음 |
...
| 공간 진단 | 부분 완료 | 공간명, 환경 점수 factor 5종, 최신 측정값, 토양 추천, 대체 작물 추천 | 개선 방안, 7일 일정, 예상 변화 문구는 정적 안내 |
| 진단 이력 | 완료 | 최근 30일 측정 샘플을 기준으로 재계산한 적합도 이력 | 측정 데이터가 없으면 이력도 없음 |
| 관리 가이드 | 부분 완료 | 토양 배합·재료·주의사항 | 오늘 작업, 재배 기준, 상품 추천은 정적 안내 |
| 상품 구매 | 부분 완료 | 상품명, 가격, 카테고리, 추천 배지 조회 | 장바구니는 로컬 UI 상태이며 결제·주문 API 없음. 백엔드 상품 카탈로그도 현재 코드 기반 목록 |
...
| 대상 | 필요한 작업 | 현재 백엔드 상태 |
| --- | --- | --- |
| 상품 장바구니·주문 | 장바구니 저장, 결제/주문 API와 화면 흐름 추가 | 대응 백엔드 API 없음 |
